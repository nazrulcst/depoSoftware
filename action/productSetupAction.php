...
if($checkName==$proName){
	...
	if(!empty($proName) && !empty($proPrice) && !empty($proQuantity) && !empty($catId)){
		$insertProduct=$db->prepare('INSERT INTO `products` SET `user_id`=?,`cat_id`=?,`pro_name`=?,`pro_price`=?,`quantity`=?,`total_price`=?,`uploader`=?,`entry_date`=?');
		...
		$insertProduct->bindParam(8,$currentDate);
